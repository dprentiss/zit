...
//
{
  ...
  /*
    printf("%u total threads in %u blocks with %u threads per block\n", numThreads, numBlocks, threadsPerBlock);
    printf("%i buyers and %i sellers attempting %i trades\n", numberOfBuyers, numberOfSellers, MaxNumberOfTrades);
...
void OpenMarket()
{
  ...
  */
  ...
  doTrades<<<numBlocks, threadsPerBlock>>>(Buyers, Sellers);
